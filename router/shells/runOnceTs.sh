...
if [ "$runtime" -gt "$uptime" ]; then
  ...
  iptables -t nat -A zone_lan_prerouting -s 192.168.31.0/24 -d 192.168.5.181/32 -p tcp -m tcp --dport 61022 -m comment --comment "redirect (reflection)" -j DNAT --to-destination 192.168.31.1:61022
